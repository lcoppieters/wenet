...
exp_dir=/idiap/temp/lcoppieters/tmp_chime4/exp/1c_wsj1
decode_modes="ctc_prefix_beam_search attention_rescoring"
average_checkpoint=false
average_num=10
checkpoint=/idiap/temp/lcoppieters/tmp_chime4/exp/1c_wsj1/epoch_12.pt
. ./path.sh
...
if [ $end -ge 2 ] && [ $beg -le 2 ]; then
  ...
  for name in dev_wsj1 train_wsj1; do
    python3 tools/text2token.py $data_dir/$name/text -n 1 -s 1 \
      -l $data_dir/train_wsj1/non_lang.txt > $data_dir/$name/char
  done
  mkdir -p $(dirname $dict) && echo -e "<blank> 0\n<unk> 1" > ${dict}
  cat $data_dir/train_wsj1/char | cut -f 2- -d" " | tr " " "\n" | sort | uniq | awk '{print $0 " " NR+1}' >> ${dict}
...
if [ $end -ge 3 ] && [ $beg -le 3 ]; then
  # ./local/chime4_gen_wav.sh $data_dir/train_wsj1 $dump_wav_dir
  # tools/compute_cmvn_stats.py --num_workers 16 \
  #  --train_config $train_config \
  #  --in_scp $data_dir/train_wsj1/wav.scp \
  #  --out_cmvn $data_dir/train_wsj1/global_cmvn
  echo "Prepare data, prepare required format"
  for x in train_wsj1 dev_wsj1; do
...
if [ $end -ge 4 ] && [ $beg -le 4 ]; then
  echo start stage 4
  mkdir -p $exp_dir && cp $data_dir/train_wsj1/global_cmvn $exp_dir
  echo here
  echo $train_config 
  echo $data_dir/train_wsj1/data.list 
  echo $data_dir/dev_wsj1/data.list 
  echo $exp_dir 
  echo $dict 
  ...
  torchrun wenet/bin/train.py \
    --config $train_config \
    --train_data $data_dir/train_wsj1/data.list \
    --cv_data $data_dir/dev_wsj1/data.list \
    --model_dir $exp_dir \
    --num_workers 4 \
    --symbol_table $dict \
